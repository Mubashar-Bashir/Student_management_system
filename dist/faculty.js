...
class Faculty {
    async addNewFaculty() {
        ...
        const newFaculty = new Faculty(facultyDetails.name, facultyDetails.specialization, facultyDetails.email, facultyDetails.contact, facultyDetails.address);
    ...
    constructor(name, specialization, email, contact, address) {
        this.lastFacultyID = 2000;
        this.facultyList = [];
    ...
    async facultyDashboard(portalRole) {
        console.clear();
        ...
        while (true) {
            ...
            switch (action) {
                case 'viewCourses':
                    ...
                    const courseList = course_obj.availableCourses.filter((course) => course.facultyName === this.name);
...
const faculty_obj = new Faculty("John Doe", // name
